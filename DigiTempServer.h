...
String SendHTML() {
  ...
  ptr += "</head>\n";
  ptr += "<body>\n";
  ptr += "<h1>ESP8266 DigiTemp</h1>\n";
  ptr += "<h3>Access Point(AP) ";
  ptr += connections;
  ptr += " clients</h3>\n";
...
//     if(clients[ii].isAlive) {
        ...
        if(Th_t.last_update - millis() > 1000){	// 10 seconds
        ptr += "<a class=\"button button-off\" href=\"/stations\"> ";
        } else {
        ptr += "<a class=\"button button-on\" href=\"/stations\"> ";
        }
        ptr += tempFs;
        ptr += "F ";
        ptr += humis;
...
void my_loop() {
  ...
  if(timeElapsed(UPDATE_MINUTES * minuteInterval)){ ///4)){
